...
function StatusBadge({ status }) {
  const map = {
    match: { label: 'Sesuai', cls: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' },
    partial: { label: 'Sebagian', cls: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300' },
    mismatch: { label: 'Tidak Sesuai', cls: 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300' },
  }
...
function AnalysisSection({ title, data }) {
  if (!data) return null
  return (
    <div className="bg-surface-2 dark:bg-slate-800 border border-border dark:border-border-dark rounded-card p-4 space-y-2">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-semibold text-text-primary dark:text-text-primary-dark">{title}</h4>
        <StatusBadge status={data.status} />
      </div>
      <p className="text-sm text-text-muted dark:text-text-muted-dark">{data.detail}</p>
      {data.suggestion && (
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3">
          <p className="text-xs font-medium text-blue-700 dark:text-blue-300 mb-0.5">Saran:</p>
          <p className="text-sm text-blue-600 dark:text-blue-200">{data.suggestion}</p>
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-text-muted dark:text-text-muted-dark">Buat ringkasan dan analisis kecocokan dengan lowongan kerja.</p>
      </div>

      <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-card p-5 space-y-4">
        <div className="flex items-center justify-between">
          <p className="text-sm font-semibold text-purple-800 dark:text-purple-300">Analisis Lowongan Kerja</p>
        ...
        <div className="flex gap-2 mb-2">
          <button
            ...
            className={`px-3 py-1.5 text-xs rounded-lg font-medium transition-colors ${jobInputMode === 'text' ? 'bg-purple-600 text-white' : 'bg-white dark:bg-slate-700 text-text-muted dark:text-text-muted-dark border border-border dark:border-border-dark'}`}
          >
            Input Teks
          </button>
          <button
            ...
            className={`px-3 py-1.5 text-xs rounded-lg font-medium transition-colors ${jobInputMode === 'image' ? 'bg-purple-600 text-white' : 'bg-white dark:bg-slate-700 text-text-muted dark:text-text-muted-dark border border-border dark:border-border-dark'}`}
          >
            Upload Gambar
          </button>
        </div>
        ...
        {jobInputMode === 'image' ? (
          <div className="space-y-2">
            <label className="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-purple-300 dark:border-purple-700 rounded-lg cursor-pointer bg-white dark:bg-slate-800 hover:border-purple-500 transition-colors">
              <div className="flex flex-col items-center gap-1">
                ...
                <span className="text-xs text-text-muted dark:text-text-muted-dark">Upload gambar lowongan (JPG/PNG)</span>
              </div>
            ...
            {ocrProgress > 0 && ocrProgress < 100 && (
              <div className="space-y-1">
                ...
                <div className="w-full h-1.5 bg-border dark:bg-border-dark rounded-full overflow-hidden">
        ...
        <div>
          ...
          <textarea
            ...
            rows={5}
            className="w-full px-4 py-2.5 bg-white dark:bg-slate-800 border border-border dark:border-border-dark rounded-lg text-body text-text-primary dark:text-text-primary-dark placeholder:text-text-muted dark:placeholder:text-text-muted-dark focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500 transition-colors resize-y"
          />
        </div>
        ...
        {showAnalysis && (
          <div className="space-y-3">
            {analyzing ? (
              <div className="py-6 text-center text-text-muted dark:text-text-muted-dark">
              ...
              <div className="space-y-3">
                {extracted && (
                  <div className="bg-white dark:bg-slate-800 border border-border dark:border-border-dark rounded-card p-4 space-y-2">
                    <h4 className="text-sm font-semibold text-text-primary dark:text-text-primary-dark">Informasi Lowongan</h4>
                    {extracted.position && (
                      <p className="text-sm"><span className="font-medium">Posisi:</span> {extracted.position}</p>
                    )}
                    {extracted.generalQualifications?.length > 0 && (
                      <div>
                        <p className="text-sm font-medium text-text-primary dark:text-text-primary-dark mb-1">Kualifikasi Umum:</p>
                        <ul className="list-disc list-inside space-y-0.5">
                          {extracted.generalQualifications.map((q, i) => <li key={i} className="text-sm text-text-muted dark:text-text-muted-dark">{q}</li>)}
                        </ul>
                      </div>
                    )}
                    {extracted.specificQualifications?.length > 0 && (
                      <div>
                        <p className="text-sm font-medium text-text-primary dark:text-text-primary-dark mb-1">Kualifikasi Khusus:</p>
                        <ul className="list-disc list-inside space-y-0.5">
                          {extracted.specificQualifications.map((q, i) => <li key={i} className="text-sm text-text-muted dark:text-text-muted-dark">{q}</li>)}
                        </ul>
                      </div>
                    )}
                  </div>
                ...
                <h4 className="text-sm font-semibold text-text-primary dark:text-text-primary-dark">Hasil Analisis Kecocokan</h4>
                ...
                {match?.overall && (
                  <div className={`rounded-card p-4 space-y-2 border ${
                    match.overall.status === 'match' ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800' :
                    match.overall.status === 'partial' ? 'bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-800' :
                    'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800'
                  }`}>
                    <div className="flex items-center justify-between">
                      <h4 className="text-sm font-semibold text-text-primary dark:text-text-primary-dark">Kesimpulan</h4>
                      <div className="flex items-center gap-2">
                        {match.overall.score != null && (
                          <span className={`text-sm font-bold ${
                            match.overall.score >= 75 ? 'text-green-600 dark:text-green-400' :
                            match.overall.score >= 50 ? 'text-yellow-600 dark:text-yellow-400' :
                    ...
                    <p className="text-sm text-text-muted dark:text-text-muted-dark">{match.overall.detail}</p>
                    {match.overall.suggestion && (
                      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3 mt-2">
                        <p className="text-xs font-medium text-blue-700 dark:text-blue-300 mb-0.5">Saran:</p>
                        <p className="text-sm text-blue-600 dark:text-blue-200">{match.overall.suggestion}</p>
                      </div>
                    )}
                  </div>
                )}
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-card p-5 space-y-4">
        <p className="text-sm font-semibold text-blue-800 dark:text-blue-300">Referensi untuk AI Ringkasan</p>
