...
export default function StepEducation({ data, onChange }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h3 className="text-h3 text-text-primary dark:text-text-primary-dark mb-1">Pendidikan</h3>
        <p className="text-sm text-text-muted dark:text-text-muted-dark mb-4">Tambahkan riwayat pendidikan Anda.</p>
      </div>
      ...
      {data.educations.map((edu, i) => (
        <div key={i} className="bg-surface-2 dark:bg-slate-800 border border-border dark:border-border-dark rounded-card p-5 space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-text-primary dark:text-text-primary-dark">Pendidikan #{i + 1}</span>
