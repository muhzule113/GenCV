import { Document, Page, Text, View, StyleSheet } from '@react-pdf/renderer'
import { formatDate, formatPeriod, getSkills } from './templateUtils'

const teal = '#0D9488'
const dark = '#134E4A'

const s = StyleSheet.create({
  page: { fontFamily: 'Helvetica', fontSize: 9.5, padding: 44, color: '#1E293B' },
  header: { flexDirection: 'row', borderBottomWidth: 2, borderBottomColor: teal, paddingBottom: 16, marginBottom: 18 },
  headerLeft: { flex: 1 },
  name: { fontSize: 20, fontWeight: 'bold', color: dark, letterSpacing: 1, marginBottom: 4 },
  jobTitle: { fontSize: 10, color: teal, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 8 },
  contactItem: { fontSize: 8, color: '#666', lineHeight: 1.6 },
  headerRight: { width: '35%', alignItems: 'flex-end', justifyContent: 'center' },
  sectionTitle: {
    fontSize: 10, fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: 1.5,
    color: teal, borderBottomWidth: 1, borderBottomColor: teal, paddingBottom: 3,
    marginTop: 14, marginBottom: 7,
  },
  summary: { fontSize: 9.5, lineHeight: 1.6, color: '#334155', marginBottom: 4 },
  bul: { marginLeft: 10, marginBottom: 2, lineHeight: 1.4, fontSize: 9, color: '#334155' },
  expHeader: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 4, marginBottom: 1, alignItems: 'baseline' },
  expPosition: { fontWeight: 'bold', fontSize: 10, color: dark },
  expDate: { fontSize: 8, color: '#888' },
  expCompany: { fontSize: 8.5, color: '#64748B', marginBottom: 2, fontStyle: 'italic' },
  eduRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 1, alignItems: 'baseline' },
  eduDegree: { fontWeight: 'bold', fontSize: 9.5, color: dark },
  eduYear: { fontSize: 8, color: '#888' },
  eduDetail: { fontSize: 8.5, color: '#64748B', marginBottom: 2 },
  eduThesis: { fontSize: 8, color: '#999', fontStyle: 'italic', marginBottom: 3, marginLeft: 1 },
  skillContainer: { flexDirection: 'row', flexWrap: 'wrap', marginLeft: 10 },
  skillItemWrapper: { width: '50%' },
  skillItem: { marginBottom: 2, lineHeight: 1.4, fontSize: 9, color: '#334155' },
  projRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 3, marginBottom: 1, alignItems: 'baseline' },
  projName: { fontWeight: 'bold', fontSize: 9.5, color: dark },
  projTech: { fontSize: 8, color: '#999', fontStyle: 'italic', marginBottom: 1 },
  projDesc: { fontSize: 8.5, color: '#334155', lineHeight: 1.3, marginBottom: 2 },
  certItem: { fontSize: 8.5, color: '#334155', lineHeight: 1.5, marginBottom: 1 },
  langItem: { fontSize: 8.5, color: '#334155', lineHeight: 1.5 },
})

export function ProfessionalTemplate({ data }) {
  const skills = getSkills(data)
  const p = data.personal || {}

  return (
    <Document>
      <Page size="A4" style={s.page}>
        <View style={s.header}>
          <View style={s.headerLeft}>
            <Text style={s.name}>{p.name || ''}</Text>
            {p.jobTitle && <Text style={s.jobTitle}>{p.jobTitle}</Text>}
          </View>
          <View style={s.headerRight}>
            {p.email && <Text style={s.contactItem}>{p.email}</Text>}
            {p.phone && <Text style={s.contactItem}>{p.phone}</Text>}
            {p.city && <Text style={s.contactItem}>{p.city}</Text>}
            {p.linkedin && <Text style={s.contactItem}>{p.linkedin}</Text>}
            {p.github && <Text style={s.contactItem}>{p.github}</Text>}
            {p.portfolio && <Text style={s.contactItem}>{p.portfolio}</Text>}
          </View>
        </View>

        {data.summary && (
          <View>
            <Text style={s.sectionTitle}>Ringkasan Profesional</Text>
            <Text style={s.summary}>{data.summary}</Text>
          </View>
        )}

        {data.experiences?.length > 0 && (
          <View>
            <Text style={s.sectionTitle}>Pengalaman Kerja</Text>
            {data.experiences.map((exp, i) => (
              <View key={i} style={{ marginBottom: 7 }}>
                <View style={s.expHeader}>
                  <Text style={s.expPosition}>{exp.position}</Text>
                  <Text style={s.expDate}>{formatPeriod(exp.startDate || exp.start_date, exp.endDate || exp.end_date, exp.isCurrent)}</Text>
                </View>
                <Text style={s.expCompany}>{exp.company}</Text>
                {(exp.description || []).map((point, j) => (
                  <Text key={j} style={s.bul}>{'•'} {point}</Text>
                ))}
              </View>
            ))}
          </View>
        )}

        {data.educations?.length > 0 && (
          <View>
            <Text style={s.sectionTitle}>Pendidikan</Text>
            {data.educations.map((edu, i) => (
              <View key={i} style={{ marginBottom: 5 }}>
                <View style={s.eduRow}>
                  <Text style={s.eduDegree}>{edu.degree}, {edu.field}</Text>
                  <Text style={s.eduYear}>{edu.startYear || edu.start_year} {'–'} {edu.endYear || edu.end_year || 'Sekarang'}</Text>
                </View>
                <Text style={s.eduDetail}>{edu.institution}{edu.gpa ? ` ${'—'} IPK: ${edu.gpa}` : ''}</Text>
                {edu.thesis && <Text style={s.eduThesis}>Tugas Akhir: {edu.thesis}</Text>}
              </View>
            ))}
          </View>
        )}

        {(skills.technical.length > 0 || skills.soft.length > 0) && (
          <View>
            <Text style={s.sectionTitle}>Keahlian</Text>
            <View style={s.skillContainer}>
              {skills.technical.map((sk, i) => (
                <View key={`t-${i}`} style={s.skillItemWrapper}>
                  <Text style={s.skillItem}>• {typeof sk === 'string' ? sk : sk.name}</Text>
                </View>
              ))}
              {skills.soft.map((sk, i) => (
                <View key={`sft-${i}`} style={s.skillItemWrapper}>
                  <Text style={s.skillItem}>• {typeof sk === 'string' ? sk : sk.name || sk}</Text>
                </View>
              ))}
            </View>
          </View>
        )}

        {data.certifications?.length > 0 && (
          <View>
            <Text style={s.sectionTitle}>Sertifikasi</Text>
            {data.certifications.map((c, i) => (
              <Text key={i} style={s.certItem}>{'•'} {c.name}{c.issuer ? ` ${'—'} ${c.issuer}` : ''}{c.date ? ` (${formatDate(c.date)})` : ''}</Text>
            ))}
          </View>
        )}

        {data.languages?.length > 0 && (
          <View>
            <Text style={s.sectionTitle}>Bahasa</Text>
            <Text style={s.langItem}>{data.languages.map((l) => `${l.name}${l.level ? ` ${'—'} ${l.level}` : ''}`).join('  •  ')}</Text>
          </View>
        )}

        {data.projects?.length > 0 && (
          <View>
            <Text style={s.sectionTitle}>Proyek</Text>
            {data.projects.map((proj, i) => (
              <View key={i} style={{ marginBottom: 5 }}>
                <View style={s.projRow}>
                  <Text style={s.projName}>{proj.name}</Text>
                  {proj.period && <Text style={s.projTech}>{proj.period}</Text>}
                </View>
                {(proj.techStack || proj.tech_stack)?.length > 0 && (
                  <Text style={s.projTech}>{(proj.techStack || proj.tech_stack).join(', ')}</Text>
                )}
                {proj.description && <Text style={s.projDesc}>{proj.description}</Text>}
              </View>
            ))}
          </View>
        )}
      </Page>
    </Document>
  )
}
