...
const styles = StyleSheet.create({
  page: { fontFamily: 'Helvetica', fontSize: 9.5, padding: 42, color: '#1F2937' },
  ...
  eduRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 3, marginBottom: 1, alignItems: 'baseline' },
  eduDegree: { fontWeight: 'bold', fontSize: 10, color: '#111827' },
  ...
  expPosition: { fontWeight: 'bold', fontSize: 9.5, color: '#111827' },
  expDate: { fontSize: 8, color: '#6B7280' },
  expCompany: { fontSize: 8.5, color: '#4B5563', marginBottom: 2, fontStyle: 'italic' },
  
  skillContainer: { flexDirection: 'row', flexWrap: 'wrap', marginLeft: 4 },
  skillItemWrapper: { width: '50%' },
  skillItem: { marginBottom: 2, lineHeight: 1.4, fontSize: 9, color: '#111827' },
  certItem: { fontSize: 8.5, color: '#374151', marginBottom: 1.5 },
  langItem: { fontSize: 8.5, color: '#374151', lineHeight: 1.4 },
})

export function FreshGraduateMinimalTemplate({ data }) {
  const skills = getSkills(data)
  const p = data.personal || {}

  // Organizations can be custom input or separate field in data.organizations
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        {/* Pendidikan dulu untuk Fresh Graduate */}
          ...
          <View>
            ...
            {data.educations.map((edu, i) => (
              <View key={i} style={{ marginBottom: 5 }}>
                ...
                {edu.thesis && <Text style={styles.eduThesis}>Tugas Akhir/Skripsi: {edu.thesis}</Text>}
              </View>
            ))}
          </View>
        )}
        ...
        {data.projects?.length > 0 && (
          <View>
            <Text style={styles.sectionTitle}>Proyek Akademik & Portofolio</Text>
        ...
        {(data.experiences?.length > 0 || orgs.length > 0) && (
          <View>
            <Text style={styles.sectionTitle}>Pengalaman Magang & Organisasi</Text>
            ...
            {orgs.map((org, i) => (
              <View key={i} style={{ marginBottom: 6 }}>
                <View style={styles.expHeader}>
                  <Text style={styles.expPosition}>{org.role || org.position}</Text>
                  <Text style={styles.expDate}>{org.period}</Text>
                </View>
                <Text style={styles.expCompany}>{org.name}</Text>
                {org.description && <Text style={styles.bul}>• {org.description}</Text>}
              </View>
            ))}
          </View>
        )}
