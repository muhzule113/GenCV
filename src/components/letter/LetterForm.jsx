...
function Section({ id, title, subtitle, icon, children, defaultOpen = true, completed = false, disabled = false }) {
 ...
 return (
 <section className={`group rounded-2xl border border-border card overflow-hidden transition-all duration-300 ${disabled ? 'opacity-60 pointer-events-none' : ''}`}>
 <button
 type="button"
 onClick={() => setOpen((o) => !o)}
 className="w-full flex items-center gap-3 px-4 py-3.5 text-left hover:bg-ink/5 transition-colors"
 >
 <div className="flex-shrink-0 w-8 h-8 rounded-lg bg-ink/10 text-ink flex items-center justify-center ring-1 ring-ink/20 ">
 {completed ? (
 ...
 <span className="text-[10px] font-semibold tracking-wider text-ink uppercase">Blok {id}</span>
 ...
 <div className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-muted transition-transform duration-300 ${open ? 'rotate-180' : ''}`}>
...
function Select({ label, required, value, onChange, children, hint }) {
 ...
 onChange={onChange}
 required={required}
 className="w-full appearance-none px-4 py-2.5 pr-10 bg-surface border border-border rounded-xl text-sm text-ink focus:outline-none focus:ring-2 focus:ring-ink/30 focus:border-ink hover:border-ink transition-all"
 >
 {children}
 </select>
...
function ToggleChip({ active, onClick, children, icon }) {
 ...
 : 'bg-surface text-ink border-border hover:border-ink hover:bg-ink '
 }`}
 >
 {icon && <span className={active ? 'text-white' : 'text-muted '}>{icon}</span>}
 <span>{children}</span>
 {active && (
 <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
 <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
...
export default function LetterForm({ form, setForm, onGenerate, onSaveDraft, saving = false, loading, hasContent, existingLetter, onViewExisting }) {
 ...
 <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-ink/5 via-ink/[0.02] to-transparent border border-ink p-5">
 <div className="absolute -right-8 -top-8 w-32 h-32 bg-ink rounded-full blur-2xl" />
 <div className="absolute -right-4 -bottom-8 w-24 h-24 bg-ink rounded-full blur-xl" />
 <div className="relative">
 <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-ink/10 text-ink text-[11px] font-semibold mb-2">
 <SectionIcon name="sparkle" className="w-4 h-4" />
 <span>AI Cover Letter</span>
 </div>
 <h3 className="text-xl font-semibold text-ink ">Data Surat Lamaran</h3>
 ...
 <p className="text-xs font-semibold text-muted uppercase tracking-wider mb-2">Identitas dari CV</p>
 <div className="space-y-1.5">
 {identityFields.map((f) => (
 <div key={f.label} className="flex text-sm">
 <span className="text-muted w-32 shrink-0">{f.label}</span>
 ...
 <div className="p-4 rounded-xl bg-surface border border-border/60 ">
 <p className="text-xs font-semibold text-muted uppercase tracking-wider mb-2">Keahlian dari CV</p>
 {cvSkills.technical.length > 0 && (
 <div className="flex flex-wrap gap-1.5 mb-2">
 {cvSkills.technical.map((s) => (
 <span key={s} className="px-2 py-0.5 bg-ink/10 text-ink rounded text-xs font-medium">{s}</span>
 ))}
 </div>
 )}
 ...
 {cvSkills.soft.map((s) => (
 <span key={s} className="px-2 py-0.5 bg-surface text-muted rounded text-xs">{s}</span>
 ))}
 </div>
 )}
 </div>
 ...
 Sumber Informasi Lowongan <span className="text-muted font-normal">(opsional)</span>
 </label>
 ...
 >
 <div>
 <p className="text-sm text-muted mb-3">Pilih dokumen yang akan dilampirkan pada surat.</p>
 ...
 checked
 ? 'border-ink bg-ink shadow-sm'
 : 'border-border hover:border-ink/40 hover:bg-ink/5'
 }`}
 >
 ...
 className="sr-only"
 />
 <span className={`text-sm ${checked ? 'text-ink font-medium' : 'text-ink '}`}>
 {opt.label}
 </span>
 </label>
 )
 ...
 <SectionIcon name="paperclip" className="w-4 h-4 text-muted " />
 <span className="text-xs font-semibold text-muted uppercase tracking-wider">Ringkasan ({attachmentsList.length})</span>
 </div>
 <ol className="space-y-1.5">
 {attachmentsList.map((label, i) => (
 <li key={i} className="flex items-start gap-2 text-sm text-ink ">
 <span className="flex-shrink-0 w-5 h-5 rounded-full bg-ink/10 text-ink text-xs font-semibold flex items-center justify-center">{i + 1}</span>
 <span>{label}</span>
 </li>
 ))}
 </ol>
 ...
 <section className="rounded-2xl border border-border card shadow-card overflow-hidden">
 <div className="px-4 py-3.5 flex items-center gap-3 border-b border-border/60 ">
 ...
 <div className="flex-1 min-w-0">
 <h4 className="text-sm font-semibold text-ink ">Poin yang Ingin Ditonjolkan</h4>
 <p className="text-xs text-muted mt-0.5">Bantu AI menonjolkan keunggulanmu yang paling relevan</p>
 </div>
 </div>

 <div className="p-4 space-y-3">
 {showRecommendBtn && (
 <div className="rounded-xl border border-dashed border-ink bg-ink/[0.03] p-4 flex flex-col sm:flex-row items-start sm:items-center gap-3">
 ...
 {isLoadingHighlights && (
 <div className="flex items-center gap-3 text-sm text-muted p-4 rounded-xl bg-surface ">
 ...
 <p className="text-xs font-semibold text-muted uppercase tracking-wider">Rekomendasi</p>
 <div className="space-y-2">
 ...
 selectedHighlights.includes(item) ? 'bg-ink border-ink ' : 'border-border bg-surface '
 }`}>
 {selectedHighlights.includes(item) && (
 <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
 <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
 ...
 {showRecommendations && (
 <div>
 <label className="block text-xs font-semibold text-muted uppercase tracking-wider mb-1.5">Poin Tambahan</label>
 <textarea
 value={customHighlight}
 onChange={(e) => setCustomHighlight(e.target.value)}
 placeholder="Tulis poin tambahan yang ingin ditonjolkan..."
 className="w-full px-4 py-2.5 bg-surface border border-border rounded-xl text-sm text-ink placeholder:text-text-muted focus:outline-none focus:ring-2 focus:ring-ink/30 focus:border-ink hover:border-ink transition-all resize-vertical min-h-[70px]"
 rows={2}
 />
 </div>
 )}
 ...
 <div className="p-4 rounded-xl bg-gradient-to-br from-primary/5 to-primary/[0.02] border border-ink ">
 <div className="flex items-center gap-2 mb-2.5">
 <SectionIcon name="sparkle" className="w-4 h-4 text-ink " />
 <span className="text-xs font-semibold text-ink uppercase tracking-wider">Poin Terpilih ({finalHighlights.length})</span>
 </div>
 <div className="flex flex-wrap gap-2">
 {finalHighlights.map((h) => (
 <span key={h} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-surface text-ink rounded-full text-sm border border-ink shadow-sm">
 ...
 <div className="sticky bottom-0 -mx-4 sm:-mx-6 px-4 sm:px-6 py-3 bg-gradient-to-t from-surface via-surface to-transparent border-t border-border/60 backdrop-blur-sm">
