...
export default function LetterEditor({ letter, pdfButton }) {
 ...
 <h3 className="text-lg font-semibold text-ink ">Hasil Surat</h3>
 {letter && (
 ...
 <div className="flex items-center justify-between gap-2 px-3 py-2 bg-surface border border-border ">
 ...
 disabled={autoFit}
 className="w-7 h-7 flex items-center justify-center rounded text-muted hover:bg-border disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
 aria-label="Zoom out"
 title="Zoom out"
 >
 <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
 ...
 type="button"
 onClick={handleReset}
 className="min-w-[48px] h-7 px-2 text-xs font-medium text-ink hover:bg-border rounded transition-colors"
 title="Reset zoom"
 >
 {Math.round(zoom * 100)}%
 </button>
 ...
 <div
 ref={containerRef}
 className="flex-1 overflow-auto bg-surface border-x border-b border-border "
 >
 ...
 <p className="text-muted ">Isi form di atas, lalu klik "Generate Surat" untuk menghasilkan surat lamaran.</p>
 </div>
 )}
 </div>
 )
