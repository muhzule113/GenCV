...
export default function TokensPage() {
  ...
  const startPayment = ({ order_id, payment_method, vaNumber, billCode, qrUrl, pkg }) => {
    ...
    expiryTimer.current = setTimeout(async () => {
      // Mark expired in DB — poll will detect and show "Pembayaran Gagal"
      try {
        await api.patch(`/api/tokens/purchase/${order_id}/expire`)
      } catch {}
    }, PAYMENT_EXPIRY_MS)
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-lg border transition-all ${
                    remaining === 0
                      ...
                      : 'bg-clip/10 border-clip/30 text-clip hover:bg-clip/20'
                  }`}
                >
                  {remaining === 0 ? (
                  ...
                  <span className="text-xs font-medium tabular-nums">
                    {remaining === 0 ? 'Kadaluwarsa' : formatCountdown(remaining)}
                  </span>
                </button>
              )}
